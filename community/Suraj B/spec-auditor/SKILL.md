...
|---|---|---|
| Skill does not activate on an obvious spec | Description missing a user-phrasing variant | Add the phrasing to the `description` field and retest triggering on a fresh session |
...
| Output changed after a model update | Silent model regression | Rerun the audit against a known test spec on the new model and compare; update `last-tested` in §9 |
...
Versioning discipline: MAJOR on breaking change to the workflow or output contract, MINOR on new checks, PATCH on fixes. Update `last-tested` only after rerunning the audit against a known test spec on the current model.
